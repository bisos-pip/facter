#+title: bisos.facter:  Adoption and adaptation of facter to Python and as Command-Services that uses the PyCS-Framework
...
The file [[file:./py3/bisos/facter/facter.py]] includes functions that run a subprocess with "facter --json",
...
prefer not to use of know about COMEEGA. In such situations facter-conv.py can be considered a
...
Implemenation of commands in facter_csu.py rely on facilities provided in facter.py.
